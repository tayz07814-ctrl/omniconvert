...
// Image outputs we can reliably produce in-browser.
const IMAGE_OUTPUTS = ['png', 'jpg', 'jpeg', 'webp', 'gif', 'bmp', 'pdf'];
...
export function getTargets(srcExt: string): FormatDef[] {
  ...
  if (src.category === 'audio') {
    return FORMATS.filter(
      (f) => (f.category === 'audio' || f.category === 'video') && f.ext !== src.ext,
    );
  }
  if (src.category === 'document') {
    return FORMATS.filter((f) =>
      ['png', 'jpg', 'jpeg', 'webp', 'gif', 'bmp'].includes(f.ext),
    );
  }
  return [];
}
